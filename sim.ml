...
let clean_process p = 
  let x = ref 0 in
  begin
    pcb_table := List.filter (fun x -> x.pid <> p.pid) !pcb_table;
    while (memory.(p.start + !x)).ins <> 'T' do
      memory.(p.start + !x) <- {ins = 'N'; n = 0; name = ""};
      x := !x + 1
    done;
    memory.(p.start + !x) <- {ins = 'N'; n = 0; name = ""}
  end
  ...
  let ind = find_process_name (String.sub newP.name 0 (String.length newP.name - 4)) 0 in
    if (ind <> -1) then
      begin
        ...
        let process = {name = String.sub newP.name 0 (String.length newP.name - 4); start = p.start; variable = 0; pid = !next_pid; ppid = 0; priority = newP.priority; arrival_time = newP.time; time = 0; pc = 0; status = 0; finish = 0} in
        next_pid := !next_pid + 1;
        pcb_table := !pcb_table @ [process]; 
        Queue.push process readyQ
      end
    else
      let process = {name = String.sub newP.name 0 (String.length newP.name - 4); start = !next_memory_index; variable = 0; pid = !next_pid; ppid = 0; priority = newP.priority; arrival_time = newP.time; time = 0; pc = 0; status = 0; finish = 0} in
